...
# 主音：名称 -> (半音偏移, 黑键标注风格)
# 小三和弦的降三音会引入降号黑键（Eb/Bb/Ab 等），
# 故 C、G、F 这三个白键根音在此统一改用降号标注。
TONIC_INFO = {
    'C': (0, 'flat'), 'G': (7, 'flat'), 'D': (2, 'sharp'), 'A': (9, 'sharp'),
...
HANDS = [
    {  # 手型一 = 紧凑型：左手根音 + 右手 So-Do-Me（五音-根音-降三音）
        'label': '紧凑型',
        'sub': '手型一',
        'desc': '左手 3指(根音) ｜ 右手 1-2-4指 五音-根音-降三音',
        'notes': [
            (-12, 3, 'L'),   # 左手根音（低八度）
            (-5, 1, 'R'),    # 右手五音（在根音下方）
            (0, 2, 'R'),     # 右手根音
    ...
    {  # 手型二 = 扩张型：左手根音五音 + 右手 根-降三-五-高八度根
        'label': '扩张型',
        'sub': '手型二',
        'desc': '左手 5指(根音)+2指(五音) ｜ 右手 1-2-3-5指 根音-降三音-五音-高八度根音',
        'notes': [
            (-12, 5, 'L'),   # 左手根音（低八度）
            (-5, 2, 'L'),    # 左手五音
            (0, 1, 'R'),     # 右手根音
...
def draw_chromatic_keyboard(svg, x0, y0, start, end, highlights, mode='flat'):
...
DEGREE_Y = KEY_Y + WHITE_H + 40    # 音级标注（根音/降三音/五音）
...
def draw_hand(svg, W, tonic_name, hand):
    ...
    title = f'{tonic_name} 小三和弦 · {hand["label"]}（{hand["sub"]}）'
...
def build_single_svg(tonic_name, hand, idx):
    ...
    path = rf'd:\Githubprojects\chords-keys-cookbook\docs\assets\images\minor-triads\{tonic_name}-hand-shape-{idx}.svg'
...
if __name__ == '__main__':
    ...
    print('All 24 minor SVGs generated!')
