...
DEGREE_Y = KEY_Y + WHITE_H + 40    # 音级标注（根音/四音/五音）
...
def draw_hand(svg, W, tonic_name, hand):
    ...
    title = f'{tonic_name} Sus4 · {hand["label"]}（{hand["sub"]}）'
...
def build_single_svg(tonic_name, hand, idx):
    ...
    path = Path(__file__).resolve().parent.parent / 'docs' / 'assets' / 'images' / 'sus4-chords' / f'{tonic_name}-hand-shape-{idx}.svg'
...
if __name__ == '__main__':
    # sus4 只掌握扩张型（手型二），统一命名为 hand-shape-2。
    for tonic_name in TONICS:
        for hand in HANDS:
            build_single_svg(tonic_name, hand, 2)
    print('All 12 sus4 SVGs generated!')
