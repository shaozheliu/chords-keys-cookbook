# ============================================================
# add9「和弦进行」手位图生成器（12 调）
# 进行骨架：IVsus2 → Vsus4 → viadd9 → viadd9（下属 sus2 → 属 sus4 → 小主 add9）
# add9 固定落在 vi 级（小主），重复一次让九音色彩沉淀。
...
PROGRESSION = [
    ...
    {  # viadd9（小主加九）· 扩张型：左手 do-so-do、右手 re-mi-so-do
        'degree': 'viadd9',
        'deg_offset': 9,
        'suffix': 'madd9',
        'notes': [
            (-3, 5, 'L', '根音'),    # 左手根音（低八度 A2）
            (4, 4, 'L', '五音'),     # 左手五音（E3）
            (9, 1, 'L', '根音'),     # 左手高八度根音（A3）
            (11, 1, 'R', '九音'),    # 右手九音（B3）
            (12, 2, 'R', '三音'),    # 右手三音（C4）
            (16, 3, 'R', '五音'),    # 右手五音（E4）
            (21, 5, 'R', '根音'),    # 右手高八度根音（A4）
        ],
        'range': (-3, 21),
    },
    {  # viadd9（重复一次，让九音色彩沉淀）
...
def build_single_svg(tonic_name):
    ...
    svg.append(f'<text x="{W/2}" y="{TITLE_Y}" text-anchor="middle" font-family="Arial,sans-serif" font-size="18" font-weight="bold" fill="#2c3e50">{tonic_name} 大调 · IVsus2 → Vsus4 → viadd9 → viadd9</text>')
    svg.append(f'<text x="{W/2}" y="{SUBTITLE_Y}" text-anchor="middle" font-family="Arial,sans-serif" font-size="12" fill="#888">下属 sus2 舒展 → 属 sus4 蓄势 → 小主 add9 收束</text>')
    ...
    path = Path(__file__).resolve().parent.parent / 'docs' / 'assets' / 'images' / 'add9-progressions' / f'{tonic_name}-progression.svg'
...
if __name__ == '__main__':
    ...
    print('All 12 add9 progression SVGs generated!')
