# ============================================================
# 自动生成 docs/_sidebar.md 的脚本（三层结构：篇 → 章 → 节）
# 约定：
...
def collect_parts():
    """扫描 docs 下的篇文件夹，返回 [(篇标题, [(章标题, 章链接|None, [(节md, 节标题)...])...])...]。"""
    ...
    for part_dir in part_dirs:
        ...
        for kind, path in items:
            if kind == 'file':
                title = extract_h1(path) or path.stem
...
def main():
    ...
    section_count = sum(len(sections) for _, chapters in parts for _, _, sections in chapters)
    print(f'已生成 {SIDEBAR_PATH}（{part_count} 篇 / {chapter_count} 章 / {section_count} 节）')
