...
# 手型一 = 收缩型（左手 do-so + 右手 la-do-mi）
# 手型二 = 扩张型（左手 do-so + 右手 do-mi-so-la）
# 命名：{根音}-hand-shape-{1|2}.svg，如 C-hand-shape-1.svg。
# 大六和弦 = 大三和弦（根音+大三度+纯五度）+ 大六度（+9）。
# ============================================================
...
# 主音：名称 -> (半音偏移, 黑键标注风格)
# 大六和弦保留大三度、不引入降三音，故黑键标注风格与大三一致（按调号）。
TONIC_INFO = {
...
HANDS = [
    {  # 手型一 = 收缩型：左手 do-so + 右手 la-do-mi（六音-根音-三音）
        'label': '收缩型',
        'sub': '手型一',
        'desc': '左手 3-1指 根音-五音 ｜ 右手 1-2-4指 六音-根音-三音',
        'notes': [
            (-12, 3, 'L'),   # 左手根音（低八度）
            (-5, 1, 'L'),    # 左手五音（so）
            (-3, 1, 'R'),    # 右手六音（la）
    ...
    {  # 手型二 = 扩张型：左手 do-so + 右手 do-mi-so-la（根音-三音-五音-六音）
        'label': '扩张型',
        'sub': '手型二',
        'desc': '左手 5-2指 根音-五音 ｜ 右手 1-2-3-4指 根音-三音-五音-六音',
        'notes': [
            (-12, 5, 'L'),   # 左手根音（低八度）
            (-5, 2, 'L'),    # 左手五音（so）
            (0, 1, 'R'),     # 右手根音（do）
...
DEGREE_Y = KEY_Y + WHITE_H + 40    # 音级标注（根音/三音/五音/六音）
...
def draw_hand(svg, W, tonic_name, hand):
    ...
    title = f'{tonic_name}6 · {hand["label"]}（{hand["sub"]}）'
    ...
    for rel, finger, side in hand['notes']:
        ...
        deg = {0: '根音', 4: '三音', 7: '五音', 9: '六音'}[rel % 12]
...
def build_single_svg(tonic_name, hand, idx):
    ...
    path = Path(__file__).resolve().parent.parent / 'docs' / 'assets' / 'images' / 'maj6-chords' / f'{tonic_name}-hand-shape-{idx}.svg'
...
if __name__ == '__main__':
    ...
    print('All 24 maj6 SVGs generated!')
